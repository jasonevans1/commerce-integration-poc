...
- [ ] `it recalculates delivery fee when checkout/updated fires with a new shipping address`
- [ ] `it does not recalculate when address country and region have not changed`
- [ ] `it recalculates when shipping country changes`
- [ ] `it recalculates when shipping region changes`
- [ ] `it does not recalculate when checkout/updated fires with no shipping address`
- [ ] `it stores previous address key to prevent unnecessary API calls`
- [ ] `it updates deliveryFeeState.fee with the new fee`
- [ ] `it sets deliveryFeeState.fee to null when no fee applies`
- [ ] `it calls cartApi.refreshCart after updating deliveryFeeState to trigger re-render`
...
- Imports `cartApi` from `@dropins/storefront-cart/api.js` (already imported in containers.js but may need to be imported here too)
- Passes lint
