...
- Related files:
  ...
  - `blocks/commerce-checkout/containers.js` -- `h` (hyperscript) import already present; use it for the fee DOM node
...
**Note:** Verify exact CSS classes by reading existing order summary styles. The `fetchDeliveryFee` return should include `{ amount, label, currency }` -- update task 004 return shape if needed.
