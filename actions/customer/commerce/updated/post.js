...
function postProcess(data, transformed, preProcessed, result) {
  ...
  const logger = Core.Logger("customer-commerce-updated-post", {
    level: "debug",
  });

  logger.info(
    `Customer updated event completed - ID: ${transformed.customerId}, type: ${transformed.eventType}, success: ${result.success}`,
  );
}
